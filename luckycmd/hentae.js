...
ezra({
  ...
  try {
    ...
    const cap = `*Screenshot taken by Davincs  Md*`;
...
ezra({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by Davincs Md*`;
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and DAVINCS-MD will extract the text for you.");
    }
    ...
    message.sendMessage(user, {
      ...
      'caption': "*powered by DAVINCS_TESCH*"
    }, {
      'quoted': messageInstance
    });
  } catch (error) {
